...
server <- function(input, output) {
  ...
  event_data_filtered <- metaReactive({event_data %>% dplyr::filter(PARAMCD == !!input$filter_param)})
  pat_selected <- metaReactive({pat_data %>%
            dplyr::select(dplyr::one_of(c(!!input$select_regressor, c("SUBJID", "STUDYID"))))})

  anl <- metaReactive({
    merge(!!pat_selected(), !!event_data_filtered(), by = c("SUBJID", "STUDYID"))
  })
  ...
  model <- metaReactive2({
    validate(need(is.data.frame(anl()), "Data Set could not be created"))
    validate(need(is.language(formula_reactive()), "Formula could not be created from column selections"))

    metaExpr({
      lm(formula = !!formula_reactive(), data = !!anl())
    })
  })
